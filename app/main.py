...
from . import admin, db, jobs, llm, memory, metrics, postcall, reminders, tools, vapi, web_tools
...
app = FastAPI(title="voice-agent", version="0.4.0", lifespan=lifespan)
...
async def vapi_webhook(request: Request, background: BackgroundTasks):
    ...
    if kind == "end-of-call-report" and call_id:
        ...
        background.add_task(postcall.email_owner, call_id)  # after the summary task above
